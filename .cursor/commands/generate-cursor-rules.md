...
6. Metadata is stored in frontmatter and controls how the rule is used and must be formatted properly.
   1. Metadata properties are:
      1. alwaysApply: true/false # Will apply to every request
      2. description: string # A description that allows the agent to fetch the rule
      3. globs: string # A comma separated list of gitignore style patterns controlling what files this rule will apply to
   2. Usually only one of the types would be set, though it is valid to have globs and description.
   3. Example rule that is always applied concerning project structure:
    ...
    [index.js](mdc:index.js), which load configuration from
